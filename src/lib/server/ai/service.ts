...
const MOCK_IMAGE_GENERATION_DELAY_MS = 1500;
...
export async function generateSocialPostPack(
  ...
  if (input.mockImageGeneration) {
    console.info('[ai-service] Image generation mock enabled. Simulating provider latency.', {
      delayMs: MOCK_IMAGE_GENERATION_DELAY_MS
    });
    await delay(MOCK_IMAGE_GENERATION_DELAY_MS);

    console.info('[ai-service] Image generation mock completed. Returning demo images.');
    return {
      mode: 'generated',
      promptPlan,
      promptGrade,
      images: demoImages(),
      warnings: [...warnings, 'Image generation mocked: skipped provider image edit request.']
    };
  }
